      ...
      uniqueCalls.map(async (call: Call) => {
        if (call.analysisUrl && call.status === 'complete') {
          try {
            ...
            // Calculate QA score average (7 core dimensions, excluding legacy compliance)
            let qaScore: number | undefined = undefined;
            if (analysis.scores) {
              const qaScores = [
              ...
              if (qaScores.length > 0) {
                qaScore = qaScores.reduce((a, b) => a + b, 0) / qaScores.length;
              }
            }
            ...
            let overallScore: number | undefined = call.overallScore; // Keep existing if we can't calculate
            if (qaScore !== undefined && complianceScore !== undefined) {
              // If complianceScore is 0, fall back to QA score only
              overallScore = complianceScore > 0
                ? (qaScore * 0.7) + (complianceScore * 0.3)
                : qaScore;
            } else if (qaScore !== undefined) {
              // If we only have QA score, use that
              overallScore = qaScore;
            }
...
export async function saveAnalysis(analysis: any): Promise<void> {
  ...
  // Calculate QA score (7 core dimensions, excluding legacy compliance)
  let qaScore: number | undefined = undefined;
  if (analysis.scores) {
    const qaScores = [
      analysis.scores.rapport,
  ...
  // Calculate compliance score (UK compliance dimensions)
  let complianceScore: number | undefined = undefined;
  if (analysis.scores) {
    const complianceScores = [
      analysis.scores.callOpeningCompliance,
  ...
  // Calculate weighted overall score (70% QA + 30% Compliance)
  let overallScore: number | undefined = analysis.overallScore;
  if (qaScore !== undefined && complianceScore !== undefined) {
    // If complianceScore is 0, fall back to QA score only
    overallScore = complianceScore > 0
      ? (qaScore * 0.7) + (complianceScore * 0.3)
  ...
  // Add calculated scores to analysis
  ...
  await fs.writeFile(jsonPath, JSON.stringify(enrichedAnalysis, null, 2), 'utf-8');
}
